...
$conn=mysqli_connect("localhost","root", "","scheduler") or die("The connection to the database has not been made");
    ...
    $activationcode =$_GET['activationcode']; // Set hash variable
                 
    $search = mysqli_query($conn,"SELECT email, activationcode, active FROM user WHERE email='".$email."' AND activationcode='".$activationcode."' AND active='0'") or die("The connection to the database has not been made"); 
    $match  = mysqli_num_rows($search);
                 
    if($match > 0){
        // We have a match, activate the account
        mysqli_query($conn,"UPDATE user SET active='1' WHERE email='".$email."' AND activationcode='".$activationcode."' AND active='0'") or die("The connection to the database has not been made");
...
<div class="modal fade" id="login-modal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style="display: none;">
    ...
    <label for="inputEmail" class="control-label">Email</label>
					...
					<input type="email" name="email" class="form-control" id="inputEmail" value="<?php echo $email; ?>" data-error="That email address is invalid" required>
