...
$conn=mysqli_connect("localhost","root", "","scheduler") or die("The connection to the database has not been made");
    ...
    $search = mysqli_query($conn,"SELECT mno, membername, active FROM attendee WHERE mno='".$mno."' AND membername='".$membername."' AND active='pending'") or die("The connection to the database has not been made"); 
    $match  = mysqli_num_rows($search);
                 
    if($match > 0){
        // We have a match, activate the account
        mysqli_query($conn,"UPDATE attendee SET active='yes' WHERE mno='".$mno."' AND membername='".$membername."' AND active='pending'") or die("The connection to the database has not been made");
        $success_message='You have accepted meeting invitation and you are going to attend the meeting.';
    }else{
